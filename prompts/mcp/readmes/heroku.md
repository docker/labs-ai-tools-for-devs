...
## Summary
1. `create_addon` Create a new Heroku add-on for an application.
1. `create_app` Create a new Heroku application with customizable settings.
...
1. `deploy_to_heroku` Deploy projects to Heroku, replaces manual git push workflows.
1. `get_addon_info` Get comprehensive information about a Heroku add-on.
1. `get_app_info` Get comprehensive information about a Heroku application.
1. `get_app_logs` View application logs with flexible filtering options.
1. `list_addon_plans` List available plans for a specific Heroku add-on service.
1. `list_addon_services` List available Heroku add-on services.
1. `list_addons` List Heroku add-ons with flexible filtering options.
1. `list_apps` List Heroku applications with flexible filtering options.
1. `list_private_spaces` List Heroku Private Spaces available to the user.
1. `list_teams` List Heroku Teams the user belongs to.
1. `maintenance_off` Disable maintenance mode for Heroku applications.
1. `maintenance_on` Enable maintenance mode for Heroku applications.
1. `pg_backups` Manage database backup operations and schedules.
1. `pg_credentials` Manage database access credentials and security.
1. `pg_info` Display detailed information about Heroku PostgreSQL databases.
1. `pg_kill` Terminate specific database processes.
1. `pg_locks` Analyze database locks and blocking transactions.
1. `pg_maintenance` Monitor database maintenance status and operations.
1. `pg_outliers` Identify resource-intensive database operations.
1. `pg_ps` Monitor active database queries and processes.
1. `pg_psql` Execute SQL queries against Heroku PostgreSQL databases.
1. `pg_upgrade` Upgrade PostgreSQL database version.
1. `pipelines_create` Create new Heroku deployment pipelines.
1. `pipelines_info` Display detailed pipeline configuration.
1. `pipelines_list` View available Heroku pipelines.
1. `pipelines_promote` Promote applications through pipeline stages.
1. `ps_list` List and monitor Heroku application dynos.
1. `ps_restart` Restart Heroku application processes.
1. `ps_scale` Scale and resize Heroku application dynos.
1. `rename_app` Rename an existing Heroku application.
1. `transfer_app` Transfer ownership of a Heroku application.
