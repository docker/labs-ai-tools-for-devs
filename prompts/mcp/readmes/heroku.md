...
-|-
`create_addon`|Create a new Heroku add-on for an application.|
`create_app`|Create a new Heroku application with customizable settings.|
`deploy_one_off_dyno`|Execute code or a command on a Heroku one-off dyno in a sandboxed environment with network and filesystem access.|
`deploy_to_heroku`|Deploy projects to Heroku, replaces manual git push workflows.|
`get_addon_info`|Get comprehensive information about a Heroku add-on.|
`get_app_info`|Get comprehensive information about a Heroku application.|
`get_app_logs`|View application logs with flexible filtering options.|
`list_addon_plans`|List available plans for a specific Heroku add-on service.|
`list_addon_services`|List available Heroku add-on services.|
`list_addons`|List Heroku add-ons with flexible filtering options.|
`list_apps`|List Heroku applications with flexible filtering options.|
`list_private_spaces`|List Heroku Private Spaces available to the user.|
`list_teams`|List Heroku Teams the user belongs to.|
`maintenance_off`|Disable maintenance mode for Heroku applications.|
`maintenance_on`|Enable maintenance mode for Heroku applications.|
`pg_backups`|Manage database backup operations and schedules.|
`pg_credentials`|Manage database access credentials and security.|
`pg_info`|Display detailed information about Heroku PostgreSQL databases.|
`pg_kill`|Terminate specific database processes.|
`pg_locks`|Analyze database locks and blocking transactions.|
`pg_maintenance`|Monitor database maintenance status and operations.|
`pg_outliers`|Identify resource-intensive database operations.|
`pg_ps`|Monitor active database queries and processes.|
`pg_psql`|Execute SQL queries against Heroku PostgreSQL databases.|
`pg_upgrade`|Upgrade PostgreSQL database version.|
`pipelines_create`|Create new Heroku deployment pipelines.|
`pipelines_info`|Display detailed pipeline configuration.|
`pipelines_list`|View available Heroku pipelines.|
`pipelines_promote`|Promote applications through pipeline stages.|
`ps_list`|List and monitor Heroku application dynos.|
`ps_restart`|Restart Heroku application processes.|
`ps_scale`|Scale and resize Heroku application dynos.|
`rename_app`|Rename an existing Heroku application.|
`transfer_app`|Transfer ownership of a Heroku application.|
