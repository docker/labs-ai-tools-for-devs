...
const spawnProcess = (client: v1.DockerDesktopClient, setStatus: (status: keyof typeof statusMap) => void) => {
    setStatus(0)
    const process = client.docker.cli.exec('run', ['--rm', '-i', '-v', 'docker-prompts:/prompts', 'vonwig/inotifywait', '-e', 'modify', '-e', 'create', '-e', 'delete', '-m', '-q', '/prompts/'], {
        stream: {
            onOutput: (data) => {
                console.log('Claude Desktop Config data: ' + data.stdout)
            },
            onError: (error) => {
                console.error('Claude Desktop Config error: ' + error)
            },
            onClose: (exitCode) => {
                console.log('Claude Desktop Config exitCode: ' + exitCode)
                switch (exitCode) {
                    case 0:
                        setStatus(1)
                        break;
...
export const ClaudeConfigStatus = ({ client }: { client: v1.DockerDesktopClient }) => {
    const [status, setStatus] = useState<keyof typeof statusMap>(0);

    useEffect(() => {
        spawnProcess(client, setStatus)
    }, []);
